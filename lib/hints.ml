...
(* Draws a single hint number within a specific
    position in a Sudoku cell.

    ctxt: The Cairo drawing context
...
let draw_hint_number ctxt x y n i hint_size =
  ...
  let hint_x = x +. (float_of_int hint_col *. Configure_ui.cell_size /. 3.) +. 5. in
  let hint_y = y +. (float_of_int hint_row *. Configure_ui.cell_size /. 3.) +. 15. in
...
(* Draws all hint numbers on the Sudoku board.
    ...
    ctxt: The Cairo drawing context
    hints: A 9x9 array where each element contains a list of valid numbers for that cell
*)
      ...
      Array.iteri
        (fun col hints ->
          ...
          then
            ...
            let x = Configure_ui.margin +. (float_of_int col *. Configure_ui.cell_size) in
            let y = Configure_ui.margin +. (float_of_int row *. Configure_ui.cell_size) in
...
let create_empty_hints () =
  Array.map (fun row -> Array.copy row) empty_hints
...
let filter_hints original_board (hints_board : int list array array) : int list array array =
  Array.iteri
    (fun row row_array ->
      Array.iteri
        (fun col _value ->
